...
const notificationMarker = `{${theme.barColor}-fg}-{/${theme.barColor}-fg}!{${
  theme.barColor
}-fg}-{/${theme.barColor}-fg}`;
...
const createChannelView = () => {
  ...
  view.on("click", function(data) {
    if (orbit && _currentChannel) {
      orbit
        .send(_currentChannel, "🐋")
        .catch(e =>
          log(
            `{${theme.higlightColor}-fg}ERROR!{/${theme.higlightColor}-fg} ${e}`
          )
        );
    }
  });
...
const logWindow = createChannelView();

const statusBar = blessed.textbox({
  bottom: 1,
  width: "50%",
  height: 1,
  tags: true,
...
const headerBar = blessed.textbox({
  top: 0,
  width: "100%",
  height: 1,
  tags: true,
...
const channelBox = blessed.textbox({
  bottom: 0,
  height: 1,
  tags: true,
  style: {
  ...
  if (characters[0] === "/") {
    ...
    if (cmd === "quit" || cmd === "q") process.exit(0);
      ...
      if (args[1]) {
        ...
        if (isChannelOpen) orbit.leave(channel);
...
const updateUI = () => {
  const channelBoxText = _currentChannel
    ? `[#${_currentChannel}] `
    : `[(orbit)] `;
  ...
  let channels = !_currentChannel
    ? `{bold}orbit{/bold}{${theme.higlightColor}-fg}${
        Object.keys(orbit.channels).length > 0 ? "|" : ""
      }{/${theme.higlightColor}-fg}`
    : `orbit{${theme.higlightColor}-fg}|{/${theme.higlightColor}-fg}`;
  channels += Object.keys(orbit.channels)
    .map(e => {
  ...
  statusBar.setContent(
    `${bracketOpen}${time}${bracketClose} ${bracketOpen}${user}${bracketClose} ${channelsInfo}`
  );
...
const addMessagesToUI = (channel, messages) => {
  Promise.map(
    ...
    post => {
      ...
      const username = `{grey-fg}<{/grey-fg} {bold}${
        user.name
      }{/bold}{grey-fg}>{/grey-fg}`;
      const line = `${getFormattedTime(post.meta.ts)} ${username} ${
        post.content
      }`;
...
const nextView = direction => {
  ...
  if (
    (_currentChannel &&
      (currentIndex === Object.keys(orbit.channels).length - 1 &&
        direction === "next")) ||
    ...
    if (direction === "next") nextIndex = currentIndex + 1;
      ...
      nextIndex = _currentChannel
        ? currentIndex - 1
        : Object.keys(orbit.channels).length - 1;
...
commandBar.setContent(
  `{right}Type ${boldText("/quit")} to exit. ${boldText(
    "Ctrl-n"
  )} and ${boldText("Ctrl-p")} to move between channels.{/right}`
);
...
ipfs.on("ready", () => {
  ...
  orbit.events.on("joined", channel => {
    ...
    _currentChannel = channel;
    log(
      `{${theme.higlightColor}-fg}{bold}${user}{/bold}{/${
        theme.higlightColor
      }-fg} has joined channel {bold}#${channel}{/bold}`
    );
    updateUI();
  });
  ...
  orbit.events.on("left", channel => {
    ...
    log(
      `{${theme.higlightColor}-fg}{bold}${user}{/bold}{/${
        theme.higlightColor
      }-fg} has left channel {bold}#${channel}{/bold}`
    );
    updateUI();
  });
